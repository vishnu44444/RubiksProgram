...
class App extends Component {
  ...
  componentDidMount() {
    ...
    let camera = new THREE.PerspectiveCamera( 61, window.innerWidth/window.innerHeight, .1, 1000 );
    ...
    function ontouchend( event ){
      ...
      if(ignoreChange) this.controls.reset();
      ignoreChange = false;
      previousPiece = null;
      this.controls.enabled = true;
    }
    ...
    window.addEventListener("pointerup", ontouchend.bind(this), false );
    ...
    this.controls.dampingFactor = 0.15;   //damping inertia
    this.controls.enableZoom = true;      //Zooming
    this.controls.autoRotate = true;     //Enable auto rotation
    this.controls.autoRotateSpeed = 2.0;
    this.controls.minDistance = (2+cD);
    this.controls.maxDistance = (2+cD)+20;
    ...
    let animate = () => {
      ...
      // If user is interacting, stop rotation and clear any resume timer.
      if (this.state.currentFunc !== 'None') {
        if (this.controls) {
          this.controls.autoRotate = false;
        }
        if (this.rotationResumeTimer) {
          clearTimeout(this.rotationResumeTimer);
          this.rotationResumeTimer = null;
        }
      } else { // User is not interacting
        // If rotation is off and there's no timer to turn it on, set one.
        if (this.controls && !this.controls.autoRotate && !this.rotationResumeTimer) {
          this.rotationResumeTimer = setTimeout(() => {
            if (this.controls) {
  ...
  render() {
    ...
    return (
      <div className="App" style={{width:"max-content"}}>
        ...
        <div style={{width:"100%",position:"absolute", top: "100px",margin:"auto",display:"flex"}}>
          ...
          [<button key="undo" className="redoUndo" style={{marginRight:"8px"}} onClick={() => this.undo()}><i className="fas fa-arrow-left"></i></button>,
          <button key="redo" className="redoUndo" style={{marginLeft:"8px"}} onClick={() => this.redo()}><i className="fas fa-arrow-right"></i></button>]
          :""
          }
          </div>
        </div>
