...
const ColorPicker = (props) => {
    ...
    function leave(){
        ...
        if(document.querySelector("#cpChangeData").data){
            ...
            if(data[0]==="Solver") {
                props.setState({activeMenu:"",currentFunc:"None",isValidConfig:false});
    ...
    function optionClick(e){
        document.querySelector(".warningPopup").style.display="flex";
                ...
                <div className="cpInfo">
                    {
                        ...
                        <div className="solveCpDiv"><button className="solveCp" onClick={()=>onSolveClick()}>
                            <strong style={{fontSize:'1rem'}}>{solveBtnText}</strong>
                        </button></div>:!props.cpErrors.length?
                        <div className="checkCpDiv"><button className="checkCp" onClick={()=>onCheckClick()}>
                            <strong style={{fontSize:'1rem'}}>{checkBtnText}</strong>
                        </button></div>:[]
                    }
                    <div className="cpErrorContainer">
            ...
            <Col style={{padding:"0px"}}>
                ...
                <div className="colorButtonContainer">
                    ...
                    <div className="colorButtonDiv" style={{paddingBottom:"0px",width:"100%"}}>
                        <button id="ColorPicker" data="Color Picker" onClick={optionClick} className="colorPicker activeMenu exitButton">Exit</button>
                    </div>
                </div>
            </Col>
        </Row>
